...
class F1(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        return self.topk_result("f1", result)

    def metric_info(self, pos_index, pos_len):
        # Precision@k
        precision = pos_index.cumsum(axis=1) / np.arange(
            1, pos_index.shape[1] + 1
        )

        # Recall@k
        recall = pos_index.cumsum(axis=1) / pos_len.reshape(-1, 1)

        # F1@k
        denom = precision + recall
        result = np.zeros_like(precision)
        mask = denom > 0
        result[mask] = 2 * precision[mask] * recall[mask] / denom[mask]
        return result
...
class HR(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        result = self.metric_info(pos_index, pos_len)
        return self.topk_result("hr", result)

    def metric_info(self, pos_index, pos_len):
        # Hit Rate = hit_num / pos_len
        return pos_index.cumsum(axis=1) / pos_len.reshape(-1, 1)
