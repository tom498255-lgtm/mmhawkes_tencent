...
from recbole_custom.utils.enum_type import ModelType
...
def get_model(model_name):
    ...
    for submodule in model_submodule:
        module_path = ".".join(["recbole_custom.model", submodule, model_file_name])
...
def get_trainer(model_type, model_name):
    ...
    try:
        return getattr(
            importlib.import_module("recbole_custom.trainer"), model_name + "Trainer"
        )
    except AttributeError:
        if model_type == ModelType.KNOWLEDGE:
            return getattr(importlib.import_module("recbole_custom.trainer"), "KGTrainer")
        elif model_type == ModelType.TRADITIONAL:
            return getattr(
                importlib.import_module("recbole_custom.trainer"), "TraditionalTrainer"
            )
        else:
            return getattr(importlib.import_module("recbole_custom.trainer"), "Trainer")
...
def get_flops(model, dataset, device, logger, transform, verbose=False):
    ...
    """
    if 'ICLRec' in str(model.__class__):
        return 1
